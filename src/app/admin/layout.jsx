...
export default function AdminLayout({ children }) {
  ...
  if (status === "loading") {
    return (
      <div className="min-h-screen flex items-center justify-center text-lg font-medium text-[#1e3a8a]">
        <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-[#1e3a8a]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 text-slate-900 font-sans">
      ...
      {/* Sidebar */}
      <aside
        className={`flex flex-col bg-[#1e3a8a] text-white transition-all duration-300 shadow-xl z-20 ${collapsed ? "w-20" : "w-72"
          }`}
      >
        {/* Top row */}
        <div className="flex items-center justify-between px-6 py-6 border-b border-white/10 bg-[#000066]">
          {!collapsed ? (
            <div className="font-bold text-xl tracking-wide text-[#1e3a8a]">MLA ADMIN</div>
          ) : (
            <div className="font-bold text-xl text-[#1e3a8a]">MLA</div>
          )}
        ...
        <header className="h-20 bg-white border-b border-gray-200 flex items-center justify-between px-8 shadow-sm z-10">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-gray-200">
              ...
              <div className="h-10 w-10 bg-gradient-to-br from-[#1e3a8a] to-orange-600 rounded-full flex items-center justify-center text-white font-bold shadow-md">
                A
              </div>
            </div>
          </div>
